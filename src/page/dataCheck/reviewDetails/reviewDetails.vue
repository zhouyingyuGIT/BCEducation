             ...
             <div class="searchDetails">
               ...
               <ul>
                 ...
                 <li class="spe-li"><span>交易账号</span><span v-if="detailsRes.commitApiDetail.identifyType =='HQCKZH'">记账账号</span>
                   <span v-if="detailsRes.commitApiDetail.identifyType =='KH'">客户账号</span>
                   <span>{{detailsRes.commitApiDetail.identifyCode}}</span></li>
...
<!--     复核模态框-->
     <review-modal ref="reviewModal"></review-modal>
<!--     拒绝复核模态框-->
     <reject-modal ref="rejectModal"></reject-modal>
   </div>
</template>

<script>
    import {mapMutations} from 'vuex'
    import {applyCommitDetail,applyFiledownload,applyMsgRead} from "../../../api/review";
    ...
    export default {
      ...
      mounted(){
        ...
        this.handleApplyMsgRead(par)

        if(this.$route.query.detailsId){
          let params = {
            id:this.$route.query.detailsId
          }
          this.handleApplyCommitDetail(params.id)
        }
      },
      methods:{
        //查询详情
        handleApplyCommitDetail(params){
          applyCommitDetail(params).then((res) => {
            console.log(res);
        ...
        openReviewModal(){
          this.$nextTick(() => {
            this.$refs.reviewModal.init('复核')
          })
        },
        //打开拒绝复核模态框
        openRejectModal(){
          this.$nextTick(() => {
            this.$refs.rejectModal.init('拒绝复核')
          })
        },
        // 消息已读
