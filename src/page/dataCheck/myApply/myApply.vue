<template>
    <div class="myApply">
      <div class="my-breadCrumb">当前位置: 首页 - 数据复合 - 我的申请</div>
      <div class="my-content">
        <h3 class="my-menu-name">我的申请</h3>
        <template>
...
<!--                    <th>申请人</th>-->
                  ...
                  <template v-for="(item,index) in applyInfoList" >
                    <tr class="my-list-table-content">
                      <td style="width: 50px;text-align: right"><span class="apply-message" v-if="commitCodeList.includes(item.applyCode)">未读</span></td>
...
<script>
  ...
  import {applyCommitList,applyApiList,applyMsgList} from "../../../api/review";
  ...
  export default {
        ...
        mounted(){
          this.updateNav('2')
          this.updateSider('2')
          this.handleApplyCommitList(this.form)
        ...
        methods:{
          ...
          //查询申请列表
          handleApplyCommitList(params){
            // this.form.commitTime = this.tempCommitTime
            applyCommitList(params).then((res) => {
              console.log(res);
          ...
          handleDetails(item){
            console.log(item);
            this.$router.push({path:'/myApply/reviewDetails',query:{detailsId:item.id,applyCode:item.applyCode}})
          },
          changePage(page){
            this.form.page = page
            this.handleApplyCommitList(this.form)
          ...
          handleMsgList(){
            applyMsgList().then((res) => {
              console.log(res);
              this.commitCodeList = res.data.commitCodeList
